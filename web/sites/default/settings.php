 ...
 * @endcode
 */
if (file_exists($app_root . '/../salts/local.salt.txt')) {
  // Load salt from a local uncommitted salt file if one exists.
  $settings['hash_salt'] =  $app_root . '/../salts/local.salt.txt';
}
else if (file_exists($app_root . '/../salts/salt.txt')) {
  // Otherwise, load salt from a committed salt file if one exists.
  $settings['hash_salt'] =  $app_root . '/../salts/salt.txt';
}
else {
  // A random committed hash salt to fall back to.
  $settings['hash_salt'] = 'B8pl8EGM0IMdyPdUCpu7m-nDX9P6HHHhDp6coxihzme12aHcTKGLAXNovYHDqOPtlaw7U_I8KQ';
}
